...
#t 然后，默认层大小为10的网络，默认70:15:15的数据划分。点击训练按钮（用莱文贝格-马夸特法），训练结束后有以下可视化结果：
...
#t 同理，都使用1000个数据点（产生自白噪音输入），但是一个是用层大小10，一个是用层大小6，一个是用层大小2，看看效果如何。测试效果时，仍用阶跃输入（见 @table-predict 表格的第一行的那个阶跃输入），看看能否跟踪。
...
#figure(
  table(
    ...
    caption: [串并联结构网络（不同层大小）拟合效果]
)

#t 发现层大小10的训练的很不错但6的误差稍大一些，但是仍然在$10^(-4)$量级，基本可以接受。但是当网络层大小仅仅为2时，不是很理想，从误差的变化频率也可以看出来（和层数多的比变慢了许多），证明误差主要来源不是随机的噪音，而是来源于系统性的（即神经网络预测的函数明显不对），误差量级很大，有$10^(-2)$。
...
#t slx文件中含有两组，一组是串并联，一组是并联（注释），一共提供了6个神经网络（分别是不同的训练样本个数训练出来的层大小为10网络，以及用同样的1000个数据训练但层大小不同的网络），测试时可以替换模块进去。
