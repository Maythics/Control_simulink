...
#t 以下，外界在0.8秒时，对于小球速度施加阶跃扰动（正向速度0.5，负向速度0.3），模拟锤子敲击小球或意外撞击。当负向施加的速度为0.4以上时，就失控了，即：负向比正向的阶跃扰动更易导致不稳定（下图也可见，负向的阶跃使得输入值变化要更大），这也符合物理图像，因为（平方）反比的关系，靠近0处相较于远离0处，有更大的变化。
...
#t 由于用普通模糊控制，为了消除余差，需要先验的稳态工作点的知识，但这就需要对于对象本身有精确的理解与建模。所以，我企图用积分器试凑得到稳态的$u_0$值，于是尝试用如下的模糊PID控制规则，其中P、I、D三个系数是模糊控制器的输出，三个参数分别划分为三挡：S、M、L（小中大，三角形函数），但是没有调参成功，估计是论域选择有问题，没试出合适的参数。
...
#t 分析原因，该系统的非线性性比较强，积分器的作用是试凑出稳态工作点，可能在这个试凑过程中已经发散了。如果比例作用弱，一开始就飞了，若比例作用大，则太振荡，如下（下图是普通PID的结果）：
